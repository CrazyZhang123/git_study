
## 1.loc和iloc区别
`loc`和`iloc`是两种不同的索引方式，它们的主要区别在于索引的依据：

1. `questions.loc[1, 'Incorrect Answers']`:
    
    - 使用标签（label）进行索引
    - 第一个参数`1`是行索引标签（即行的名称或索引值）
    - 第二个参数`'Incorrect Answers'`是列名
    - 这种方式通过行标签和列名来定位数据
2. `questions.iloc[1, 'Incorrect Answers']`:
    
    - 使用整数位置进行索引
    - 第一个参数`1`是行的整数位置（从0开始计数）
    - 第二个参数应该是列的整数位置，而不是列名
    - 注意：`iloc`通常不接受列名作为参数，如果要使用列名，应该用`loc`

# 2、a\[:,\[1 2]] 和a\[range(len(a)),\[1 2 ]] 区别

outputs\[:, prompt_ids.squeeze(0)].shape 与 outputs\[range(outputs.shape\[0]), prompt_ids.squeeze(0)].shape

这两种PyTorch索引方式在结果形状和含义上有重要区别：

1. **`outputs[:, prompt_ids.squeeze(0)]`**:
    
    - 这是基本的切片索引
    - `:`表示选择所有行
    - `prompt_ids.squeeze(0)`是一个一维张量，包含要选择的列索引
    - 结果形状为`(outputs.shape[0], prompt_ids.squeeze(0).shape[0])`
    - 这会创建一个二维矩阵，其中包含所有行在指定列索引位置的值
    - 例如，如果`outputs`形状为`(5, 10)`，`prompt_ids.squeeze(0)`为`[1, 3, 2]`，则结果形状为`(5, 3)`
2. **`outputs[range(outputs.shape[0]), prompt_ids.squeeze(0)]`**:
    
    - 这是高级索引（advanced indexing）
    - `range(outputs.shape[0])`创建一个行索引序列`[0, 1, 2, ..., outputs.shape[0]-1]`
    - `prompt_ids.squeeze(0)`是列索引序列
    - 两个索引序列长度必须相同
    - 结果形状为`(outputs.shape[0],)`，是一维张量
    - 这种索引方式会返回一个一维张量，其中第i个元素是`outputs[i, prompt_ids.squeeze(0)[i]]`
    - 例如，如果`outputs`形状为`(5, 10)`，`range(outputs.shape[0])`为`[0, 1, 2, 3, 4]`，`prompt_ids.squeeze(0)`为`[1, 3, 2, 0, 4]`，则结果形状为`(5,)`，包含元素`[outputs[0,1], outputs[1,3], outputs[2,2], outputs[3,0], outputs[4,4]]`

**关键区别**：

- 第一种方式返回一个二维矩阵，包含所有行在指定列的值
- 第二种方式返回一个一维向量，只包含对角线上的元素（按索引对应关系）


# 3、pd.to_csv方法

```
model_outputs = pd.read_csv('./helpsteer/helpfulness_model_outputs_seed42_n100.csv')
```



```
# iloc[index]这样取只有一个元素就会变成一个series，是竖着的
model_outputs.iloc[0].to_csv('./_outputs_seed42_n100_test.csv', index=False)
# iloc[index:index+1]这样取，还是一个df
model_outputs.iloc[0:1].to_csv('./_outputs_seed42_n100_test.csv', index=False)
# model_outputs.iloc[0]['prompt']
```


<<<<<<< HEAD
# 4、df.groupby
### 🧠 背景

BBQ 数据集是一个多类别（multi-category）偏见评估数据集。  
例如，它可能包含以下类别：

`age gender_identity race_ethnicity nationality religion ...`

不同类别的数据量可能相差很大，比如：

|category|样本数|
|---|---|
|age|500|
|gender_identity|1200|
|race_ethnicity|800|
|...|...|

如果我们直接合并使用，会导致模型在训练或评估时被“大样本类”主导，结果不平衡。  
所以我们希望让每个类别在整体中**占一样的比例（均衡采样）**。

### 🧩 实现方式

关键代码是这行：

`balanced_df = df.groupby('category', group_keys=False).apply(lambda x: x.sample(n=min_samples, random_state=42))`

让我们拆开来看👇

#### ① `df.groupby('category')`

- 这会把整个 DataFrame 按照 `category` 列分成多个小组。
    
- 每个组只包含该类别的数据。
    
- 例如：
    - group1: 所有 `age` 类的数据
    - group2: 所有 `gender_identity` 类的数据
    - group3: 所有 `race_ethnicity` 类的数据
        

#### ② `.apply(lambda x: x.sample(n=min_samples, random_state=42))`

- 对每个组执行采样操作：
    - `x` 是当前类别的数据。
    - `.sample(n=min_samples)` 随机抽取 `min_samples` 条样本。
        
- 因为我们事先计算了：
    `min_samples = min(df['category'].value_counts())`
    
    所以每一类都被下采样到 **相同的样本数**。

这样就实现了**类别均衡（balanced sampling）**：  
所有类别的数据量一致，不会有偏斜。

#### ③ `group_keys=False`

- 避免 Pandas 在分组后把类别名作为多级索引的上层 index，保持 DataFrame 扁平结构，方便后续保存。

# df.to_dict 中文总结
`DataFrame.to_dict()` 用于将 DataFrame 转换为字典（或指定的可变映射类型），核心通过 `orient` 参数控制转换格式，支持灵活的结构定制。


## 核心参数说明
| 参数       | 作用描述                                                                 |
|------------|--------------------------------------------------------------------------|
| `orient`   | 控制返回字典的结构（必选，默认 `'dict'`），支持 7 种取值（见下方详细说明） |
| `into`     | 指定返回的映射类型（默认 `dict`），支持 `OrderedDict`、`defaultdict` 等    |
| `index`    | 是否包含索引信息（默认 `True`），仅 `orient='split'/'tight'` 时可设为 `False` |


## 关键 `orient` 格式详解（含示例）
先定义基础 DataFrame 用于演示：
```python
import pandas as pd

df = pd.DataFrame({
    'col1': [1, 2],
    'col2': [0.5, 0.75]
}, index=['row1', 'row2'])  # 索引为行名
print(df)
#       col1  col2
# row1     1  0.50
# row2     2  0.75
```

### 1. 默认格式：`orient='dict'`（列→{索引→值}）
最常用格式，外层键为**列名**，内层键为**索引**，值为对应单元格数据。
```python
df.to_dict()  # 等价于 df.to_dict('dict')
# 输出：
# {'col1': {'row1': 1, 'row2': 2}, 'col2': {'row1': 0.5, 'row2': 0.75}}
```

### 2. `orient='list'`（列→[值列表]）
外层键为**列名**，值为该列所有数据组成的列表（按行顺序），忽略索引。
```python
df.to_dict('list')
# 输出：
# {'col1': [1, 2], 'col2': [0.5, 0.75]}
```

### 3. `orient='series'`（列→Series 对象）
外层键为**列名**，值为该列对应的 `Series`（保留索引）。
```python
df.to_dict('series')
# 输出：
# {'col1': row1    1
#          row2    2
# Name: col1, dtype: int64,
#  'col2': row1    0.50
#          row2    0.75
# Name: col2, dtype: float64}
```

### 4. `orient='split'`（拆分索引、列名、数据）
返回包含 3 个键的字典：`'index'`（索引列表）、`'columns'`（列名列表）、`'data'`（数据二维列表）。
```python
df.to_dict('split')
# 输出：
# {'index': ['row1', 'row2'],
#  'columns': ['col1', 'col2'],
#  'data': [[1, 0.5], [2, 0.75]]}

# 不包含索引（仅 split/tight 支持）
df.to_dict('split', index=False)
# 输出：{'columns': ['col1', 'col2'], 'data': [[1, 0.5], [2, 0.75]]}
```

### 5. `orient='tight'`（拆分+索引/列名的名称）
在 `split` 基础上，额外包含 `'index_names'`（索引的名称）和 `'column_names'`（列的名称），适合保留完整元信息。
```python
df.to_dict('tight')
# 输出：
# {'index': ['row1', 'row2'],
#  'columns': ['col1', 'col2'],
#  'data': [[1, 0.5], [2, 0.75]],
#  'index_names': [None],  # 索引无名称时为 None
#  'column_names': [None]} # 列无名称时为 None
```

### 6. `orient='records'`（行→字典列表）
返回列表，每个元素是一个字典，对应一行数据（键为列名，值为单元格数据），适合批量处理单行数据。
```python
df.to_dict('records')
# 输出：
# [{'col1': 1, 'col2': 0.5}, {'col1': 2, 'col2': 0.75}]
```

### 7. `orient='index'`（索引→{列名→值}）
与默认格式相反，外层键为**索引**，内层键为**列名**，值为对应单元格数据。
```python
df.to_dict('index')
# 输出：
# {'row1': {'col1': 1, 'col2': 0.5}, 'row2': {'col1': 2, 'col2': 0.75}}
```


## `into` 参数：自定义映射类型
默认返回 `dict`，可指定其他映射类型（如 `OrderedDict`、`defaultdict`）：

### 示例 1：OrderedDict（保留顺序）
```python
from collections import OrderedDict

# 转换后所有层级均为 OrderedDict
df.to_dict(into=OrderedDict)
# 输出：
# OrderedDict([('col1', OrderedDict([('row1', 1), ('row2', 2)])),
#              ('col2', OrderedDict([('row1', 0.5), ('row2', 0.75)]))])
```

### 示例 2：defaultdict（默认值类型）
需先初始化 `defaultdict`，指定默认值类型（如 `list`）：
```python
from collections import defaultdict

# 初始化默认值为 list 的 defaultdict
dd = defaultdict(list)
# 结合 records 格式，每个元素为 defaultdict
df.to_dict('records', into=dd)
# 输出：
# [defaultdict(list, {'col1': 1, 'col2': 0.5}),
#  defaultdict(list, {'col1': 2, 'col2': 0.75})]
```


## 核心场景推荐
| 场景需求                  | 推荐 `orient` 取值 |
|---------------------------|--------------------|
| 按列获取数据（保留索引）  | `'dict'`（默认）   |
| 按列获取纯数据列表        | `'list'`           |
| 批量处理单行数据（如接口）| `'records'`        |
| 分离索引、列名、数据      | `'split'`          |
| 保留完整元信息（含名称）  | `'tight'`          |
| 按行名（索引）获取数据    | `'index'`          |
=======
>>>>>>> 4978d2d (2025-10-26 update)
